...
counts_file = Path("/g/data/lf10/mb1232/nanopore_data/2024_neurotranscriptomics/nanocount/results/TRP_ReadCounts_cortex_reps_with_mean_FROM_NanoCount.tsv")
...
trp = pd.read_csv(counts_file, sep="\t")
...
if "Counts_mean" not in merged.columns:
    raise ValueError("Counts_mean column not found in counts TSV.")
...
save_cols = ["GeneName", "family"] + [c for c in ["Counts_rep1","Counts_rep2","Counts_rep3","Counts_mean"] if c in merged.columns]
...
def nice_step(x):
    if x <= 500:   return 50
    if x <= 2000:  return 200
    if x <= 5000:  return 500
    if x <= 20000: return 2000
    if x <= 50000: return 5000
    return 10000

yticks_step = nice_step(max_val) if max_val > 0 else 50
...
sns.barplot(
    x="label", y="Counts_mean", hue="family",
