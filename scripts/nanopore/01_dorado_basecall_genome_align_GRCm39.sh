...
# Input:  config/cortex_samples_nanopore.tsv (sample_id, pod5_dir, out_dir, genome_fa, tx_fa)
...
tail -n +2 "$SAMPLES_TSV" | while IFS=$'\t' read -r sample pod5_dir out_dir genome_fa tx_fa; do
  mkdir -p "$out_dir"
  ...
  minimap2 -a -x splice -y -k 14 -t 104 "$genome_fa" \
    "${out_dir}/${sample}.dorado_polyA_m6A_all_reads.fastq" \
    | samtools view -@ 104 -b -F 260 \
    | samtools sort -@ 104 -o "${out_dir}/${sample}.genome.primary.sorted.bam" -
